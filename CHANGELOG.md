...
- Major Windows launch hardening features:
  - Command execution overrides
  - Improved fallback chains
  - Enhanced diagnostics
  - New documentation for Windows specific behaviors

## [1.2.2] - 2026-04-09

### Fixed

- Updated package repository metadata to match the renamed GitHub repository (`capyBearista/gemini-researcher-mcp`) so npm provenance verification can validate release source correctly.
- Corrected package links in `package.json` (`repository.url`, `bugs.url`, `homepage`) to the new repository path.

### Changed

- Updated repository metadata references in `server.json` and project documentation constants to the renamed repository URL.
